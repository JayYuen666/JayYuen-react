...
const getUserFromCommit = ((commitCache) => async (sha) => {
  try {
    ...
    const { data } = await axios.get(`https://api.github.com/repos/JayYuen666/vue-app/commits/${sha}`);
...
const getIssueById = ((cache) => async (id) => {
  ...
  try {
    const { data } = await axios.get(`https://api.github.com/repos/JayYuen666/vue-app/issues/${id}`);
...
const getReleaseInfo = ((releaseCache) => async (tag) => {
  ...
  const command = isUnreleasedTag ?
    ...
    `npx auto-changelog ${version ? '--starting-version ' + version + ' --ending-version ' + version : ''
